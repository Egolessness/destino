...
/**
 * raft delete request processor.
 *
 * @author zsmjwk@outlook.com (wangkang)
 */
public class JRaftDeleteRequestProcessor implements RpcProcessor<DeleteRequest> {

    private final RaftGroupContainer raftGroupContainer;

    public JRaftDeleteRequestProcessor(RaftGroupContainer raftGroupContainer) {
        this.raftGroupContainer = raftGroupContainer;
    }

    @Override
    public void handleRequest(RpcContext context, DeleteRequest request) {
            ...
            if (raftGroup.isLeader()) {
                ...
                future.whenComplete(((response, throwable) -> {
                    if (Objects.nonNull(throwable)) {
                        Loggers.PROTOCOL.error("An error occurred while apply delete request.", throwable);
                ...
                context.sendResponse(ResponseSupport.failed("The node is not a leader." + request.getCosmos().getDomain()));
            }
        } catch (Throwable e) {
            Loggers.PROTOCOL.error("An error occurred while handle delete request.", e);
    ...
    public String interest() {
        return DeleteRequest.class.getCanonicalName();
    }
}
