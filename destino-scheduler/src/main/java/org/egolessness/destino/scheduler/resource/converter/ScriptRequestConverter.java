...
import org.egolessness.destino.scheduler.resource.ScriptResource;
...
/**
 * request converter of script {@link ScriptResource}
 *
 * @author zsmjwk@outlook.com (wangkang)
 */
public class ScriptRequestConverter implements RequestConverterFunction {
    ...
    public @Nullable Object convertRequest(@Nonnull ServiceRequestContext ctx,
        ...
        Object convertObj;
        if (expectedResultType == Script.class) {
            convertObj = serializer.deserialize(httpRequest.content().array(), Script.class);
        } else if (expectedResultType == ScriptDetailRequest.class) {
            ScriptDetailRequest scriptDetailRequest = new ScriptDetailRequest();
            scriptDetailRequest.setId(NumberUtils.parseInt(ctx.queryParam("id"), 0));
            scriptDetailRequest.setVersion(NumberUtils.parseInt(ctx.queryParam("version"), 0));
            convertObj = scriptDetailRequest;
        } else {
