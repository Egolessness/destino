...
import org.egolessness.destino.scheduler.resource.ExecutionResource;
...
/**
 * request converter of scheduled name {@link ExecutionResource}
 *
 * @author zsmjwk@outlook.com (wangkang)
 */
public class ExecutionRequestConverter implements RequestConverterFunction {
    ...
    public @Nullable Object convertRequest(@Nonnull ServiceRequestContext ctx,
        ...
        Object convertObj;
        if (expectedResultType == ExecutionTerminateRequest.class) {
            convertObj = serializer.deserialize(httpRequest.content().array(), ExecutionTerminateRequest.class);
        } else if (expectedResultType == ExecutionClearRequest.class) {
            convertObj = serializer.deserialize(httpRequest.content().array(), ExecutionClearRequest.class);
        } else {
